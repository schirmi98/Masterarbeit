...
month=['01']#['01','02','03']
x=0#ist null wenns bei 18.01 beginnt., sonst jeden tag später entspricht plus 1
cd='../../Eureka/Daten/Dship/meteorolog_daten/'
...
#        flag.to_csv('mask_{day}{month}.csv'.format(day=d,month=m))
...
#                    mask_stop3[mask_stop3==0.]=np.nan
                    ...
                    usatdata_top=pd.read_csv('topusatdata_'+d+m+'.csv', delimiter=",", decimal='.',usecols =['D'])
                    usatdata_bug=pd.read_csv('bugusatdata_'+d+m+'.csv', delimiter=",", decimal='.',usecols =['D'])
                    mask_bug_winddir=(usatdata_bug['D'] > 100.) & (usatdata_bug['D'] <300.)
...
#                    mask_stop2.reset_index(drop=True, inplace=True)
                    ...
                    mask_top_winddir=(usatdata_top['D'] > 30.) & (usatdata_top['D'] <340.)
...
#                    [[2-2.5],[2.5-3],[3.5-4],[3.5-2.5],[2.5-2],[1.5],[1.5],[1-1.5],[1-1.5],[1.5-2],[1.5-2],[1.5],[1.5-2],[1.5-2],[1.5-2],[1.5],[1.5-2],[1.5-2],[2],[2.5],[3],[2.5-3],[2.5],[2.5-3],[2.5-3],[2.5-3],[2.5-3],[2.5-3],[2-2.5],[1.5-2],[1.5-2],[2.5],[2.5-3(im Landschutz nur 1-1.5)],[3.5],[2.5],[2.5],[1.5-2],[2.5],[nur vorhersage:2-2.5],[nur vorhersage:2],[],[],[]]
                    mask_swell=[[9],[11],[15],[12],[9],[6],[6],[5],[5],[7],[7],[6],[7],[7],[7],[6],[7],[7],[8],[10],[12],[11],[10],[11],[11],[11],[11],[11],[9],[7],[7],[10],[11],[14],[10],[10],[7],[10],[9],[8],[np.nan],[np.nan],[np.nan]]
...
#                    despiking_bug=pd.read_csv('despiking_{day}{month}_bug.csv'.format(day=d,month=m))
#                    despiking_bug=despiking_bug.drop(['Unnamed: 0','X', 'Y', 'Z', 'T', 'CC','CH','V','D','AX','AY','AZ'],axis=1)
                    despiking_top=pd.read_csv('despiking_{day}{month}_top.csv'.format(day=d,month=m))
